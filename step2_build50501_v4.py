#!/usr/bin/env python3
# build50501_v4.py  — 50501 Substack builder (COPY MODE from buildsubstack_v4.py)
...
def _parse_llm_events_canonical(text: str, *, article_url: str, logger=None) -> List[Dict[str, Any]]:
    ...
    for bidx, block in enumerate(blocks, 1):
        ...
        if attacks_line:
            # try to normalize JSON-ish list: ["a", "b"]
...
def run_builder(
    *,
    ...
    """
    Standard Substack builder for 50501 (COPY MODE).
    Prompt is built ONLY by prompts_v4.compose_system_prompt(source),
    which already glues together: channel-specific preface → attacks preface → canonical protocol.
    """
    ...
    # 👇 single prompt, written to disk for debugging
    ...
    for rec in results:
        ...
        for ev in evs:
            ...
            # 👇 even if LLM forgot the line, we keep the field — schema stays stable
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Democracy Clock V4 — 50501 Substack builder (COPY MODE)")
    ap.add_argument("--source", required=True, help="Substack source key (use '50501')")
