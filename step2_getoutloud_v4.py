...
HARVESTER_ID = "outloud"
API_URL = "https://kziegler.substack.com/api/v1/archive"
...
def _discover(
    ...
    *,
    ...
    logger.info(
        "Starting OutLoud archive fetch: window %s → %s (max_pages=%d, per_page=%d)",
    ...
    while page_idx < max_pages:
        ...
        if not isinstance(posts, list):
            logger.warning(
                ...
                page_idx + 1,
                list(payload.keys())[:10] if isinstance(payload, dict) else type(payload),
            )
            break
        ...
        for p in page_posts:
            ...
            if series:
                logger.debug(
                    "Applied series-date adjustment (–1 day) for %r → anchor=%s (from published=%s)",
            ...
            if start_d <= d_pub <= end_d:
                ...
                # OUTLOUD: unlike Meidas, we KEEP ALL in-window posts
                ...
                if key not in seen_keys:
                    ...
                    logger.debug(
                        "KEPT (outloud) title=%r published=%s anchor=%s url=%s",
                        title, d_pub.isoformat(), anchor.isoformat(), url
                    )
                else:
                    audit[-1]["decision"] = "SKIPT(dup_key)"
                    logger.debug(
                        "SKIPT(reason=dup_key) title=%r published=%s url=%s",
                ...
                logger.debug(
                    "SKIPT(reason=%s) title=%r published=%s url=%s",
...
def _to_entity_v4(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "raw_line": f"[outloud] {title} ({post_date or ''})",
    }
...
def run_harvester(
    ...
    logger.info("Discovering OutLoud (COPY mode via API): %s", API_URL)
        ...
        "items_snapshot": [
            {
                ...
                "doc_type": "news_article",
                "raw_line": f"[outloud_raw] {_title_of(it)}",
            }
            for it in all_seen
        ],
    }
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Democracy Clock V4 — OutLoud harvester")
