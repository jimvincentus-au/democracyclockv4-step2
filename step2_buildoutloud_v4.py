#!/usr/bin/env python3
# buildoutloud_v4.py  — Out Loud / Kate Ziegler Substack builder (COPY MODE from buildsubstack_v4.py / build50501_v4.py)
...
def run_builder(
    *,
    ...
    """Standard Substack builder for Out Loud (COPY MODE)."""
    ...
    for rec in results:
        ...
        for ev in evs:
            ...
            # overwrite tags only if missing or empty
            if not ev.get("tags"):
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Democracy Clock V4 — Out Loud Substack builder (COPY MODE)")
    ap.add_argument("--source", required=True, help="Substack source key (use 'outloud')")
