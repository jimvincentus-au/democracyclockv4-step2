#!/usr/bin/env python3
# step2_buildtracker_v4.py  — Trump Tyranny Tracker Substack builder (COPY MODE)
...
def _parse_llm_events_canonical(text: str, *, article_url: str, logger=None) -> List[Dict[str, Any]]:
    """Parse canonical LLM output into event dicts."""
...
def run_builder(
    *,
    ...
    """Standard Substack builder for the Trump Tyranny Tracker source (COPY MODE)."""
    ...
    # Backward-compat: earlier get-tracker runs used 'tyrannytracker' as the file prefix.
    if not p.exists() and source == "tracker":
        alt = artifacts / "json" / f"tyrannytracker_filtered_{start}_{end}.json"
        if alt.exists():
            p = alt
    if not p.exists():
    ...
    items_any: Any = payload["entities"] if isinstance(payload, dict) and "entities" in payload else payload
    items: List[Dict[str, Any]] = cast(List[Dict[str, Any]], items_any)
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Democracy Clock V4 — Trump Tyranny Tracker builder")
    ap.add_argument("--source", default="tracker", help="Source key (default: tracker)")  # standardized key
