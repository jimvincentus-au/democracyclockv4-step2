...
HARVESTER_ID = "noah"
API_URL = "https://www.noahpinion.blog/api/v1/archive"
...
def _discover(
    ...
    *,
    ...
    logger.info(
        "Starting Noahpinion archive fetch: window %s → %s (max_pages=%d, per_page=%d)",
        ...
        for p in page_posts:
            ...
            if start_d <= d_pub <= end_d:
                ...
                if key not in seen_keys:
                    ...
                    logger.debug(
                        "KEPT (noah) title=%r published=%s anchor=%s url=%s",
...
def _to_entity_v4(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "raw_line": f"[noah] {title} ({post_date or ''})",
    }
...
def run_harvester(
    ...
    logger.info("Discovering Noahpinion (COPY mode via API): %s", API_URL)
        ...
        "items_snapshot": [
            {
                ...
                "doc_type": "news_article",
                "raw_line": f"[noah_raw] {_title_of(it)}",
            }
            for it in all_seen
        ],
    }
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Democracy Clock V4 — Noahpinion harvester")
